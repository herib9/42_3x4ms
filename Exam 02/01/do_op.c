#include <unistd.h>
#include <stdlib.h>
#include <stdio.h>

void	do_op(char *num1, char *sign, char *num2)
{
	int	n1 = atoi(num1);
	int 	n2 = atoi(num2);
	int	result;

	if (*sign == '+')
		result = n1 + n2;
	if (*sign == '-')
		result = n1 - n2;
	if (*sign == '*')
		result = n1 * n2;
	if (*sign == '/')
		result = n1 / n2;
	if (*sign == '%')
		result = n1 % n2;
	printf("%i\n", result);
}

int	main(int ac, char **av)
{
	if (ac == 4)
		do_op(av[1], av[2], av[3]);
	else
		write(1, "\n", 1);
}

/*Assignment name  : do_op
Expected files   : do_op.c
Allowed functions: atoi, printf, write
--------------------------------------------------------------------------------

Escribe un programa que tome tres strings:
- El primero y el tercero son representaciones de enteros con signo en base 10
  que caben en un int.
- El segundo es un operador aritmético elegido de: + - * / %

El programa debe mostrar el resultado de la operación aritmética solicitada,
seguido de una nueva línea. Si el número de parámetros no es 3, el programa
simplemente muestra una nueva línea.

Puedes asumir que las cadenas no tienen errores ni caracteres extraños. Los 
números negativos, en la entrada o salida, tendrán un y solo un '-' inicial. 
El resultado de la operación cabe en un int.

Ejemplos:

$> ./do_op "123" "*" 456 | cat -e
56088$
$> ./do_op "9828" "/" 234 | cat -e
42$
$> ./do_op "1" "+" "-43" | cat -e
-42$
$> ./do_op | cat -e
$
--------------------------------------------------------------------------------*/
